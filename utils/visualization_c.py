...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, box, conf, cls):
        ...
        for bb, cf, cl in zip(box, conf, cls):
            ...
            cent_x = int((x_min+x_max)/2)
            cent_y = int((y_min+y_max)/2)
            centre_cood = (cent_x,cent_y)
            cv2.rectangle(img, (x_min, y_min), (x_max, y_max), color, 2)
            txt_loc = (max(x_min+2, 0), max(y_min+2, 0))
            cls_name = self.cls_dict.get(cl, 'CLS{}'.format(cl))
            B.append(centre_cood)
            txt = '{} {:.2f}'.format(cls_name, cf)
            img = draw_boxed_text(img, txt, txt_loc, color)
        return img,B
		
    def draw_bboxes_switches(self, img, box, conf, cls):
        """Draw detected bounding boxes on the original image."""
        B=[]
        for bb, cf, cl in zip(box, conf, cls):
            cl = int(cl)
    ...
    def draw_circles(self, img, box, conf, cls):
        """Draw detected bounding boxes on the original image."""
        B=[]
        for bb, cf, cl in zip(box, conf, cls):
            cl = int(cl)
            ...
            cent_y = int((y_min+y_max)/2)
            centre_cood = (cent_x,cent_y)
            cv2.circle(img, centre_cood, 60, color, 2)
            txt_loc = (max(x_min+2, 0), max(y_min+2, 0))
            cls_name = self.cls_dict.get(cl, 'CLS{}'.format(cl))
            B.append(centre_cood)
            txt = '{} {:.2f}'.format(cls_name, cf)
